...
| Enforce greppable invariants | patterns this repository refuses, one rule per decision behind it | leg invariants; leg package boundaries | no | #60; #67 | six rules, each naming the record that argued it. Three of them had no subject in this tree on the run that landed them, because the loader, the constant table and the renderer are not written, and the leg prints which rules those are on every green run rather than letting the count read as cover |
